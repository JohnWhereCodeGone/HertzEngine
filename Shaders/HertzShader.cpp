...
Shader::Shader(const char* fragmentPath, const char* vertexPath)
{
	...
	const char* vShaderCode = vertexCode.c_str(); //converts from string to char array
	...
	if (!success)
	{
		...
		std::cout << "COMPILATION FAILED: VERTEX SHADER" << infoLog << std::endl;
	}
	...
	if (!fSuccess)
	{
		...
		std::cout << "COMPILATION FAILED: FRAGMENT SHADER" << fInfoLog << std::endl;
	}
	...
	if (!success)
	{
		...
		std::cout << "LINKING FAILED: SHADER PROGRAM" << infoLog << std::endl;
	}
